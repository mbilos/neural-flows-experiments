...
def download_data(year_start, year_end):
    ...
    filename = DATA_DIR / 'earthquakes'
    filename.mkdir(parents=True, exist_ok=True)
    filename = filename / f"{year_start}_{year_end}.csv"
    urllib.request.urlretrieve(url, filename)
    return filename


def preprocess():
    df = pd.read_csv(DATA_DIR / "earthquakes/1990_2019.csv")
    ...
    np.savez(DATA_DIR / "earthquakes_jp.npz", **sequences)
...
if __name__ == "__main__":
    ...
    # combine csv files
    with open(DATA_DIR / 'earthquakes/1990_2019.csv', 'wb') as wfd:

...
def variable_time_collate_fn(batch, args, device = torch.device('cpu'), data_type = 'train',
	...
	combined_tt = torch.stack([torch.cat([x[1], torch.zeros(max_len - l)], 0) for x,l in zip(batch, lens)], 0)
	combined_vals = torch.stack([torch.cat([x[2], torch.zeros(max_len - l, D)], 0) for x,l in zip(batch, lens)], 0)
	combined_mask = torch.stack([torch.cat([x[3], torch.zeros(max_len - l, D)], 0) for x,l in zip(batch, lens)], 0)
	combined_labels = torch.stack([torch.tensor(float('nan')) if x[4] is None else x[4] for x in batch], 0).to(device)
	combined_labels = combined_labels.unsqueeze(-1)

	# Normalize data an time
	combined_vals, _, _ = utils.normalize_masked_data(combined_vals, combined_mask, data_min, data_max)
	combined_tt = combined_tt / 48.
...
if __name__ == '__main__':
	...
	dataset = PhysioNet(DATA_DIR / 'physionet', train=False, download=True)

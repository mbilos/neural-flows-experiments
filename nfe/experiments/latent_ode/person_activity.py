...
def variable_time_collate_fn_activity(batch, args, device = torch.device('cpu'), data_type = 'train'):
	...
	combined_tt = torch.stack([x[1] for x in batch], 0)
	combined_vals = torch.stack([x[2] for x in batch], 0)
	combined_mask = torch.stack([x[3] for x in batch], 0)
	combined_labels = torch.stack([x[4] for x in batch], 0)

	# Normalize time 0-1
	combined_tt = combined_tt / 227.

	data_dict = {
		'data': combined_vals, # (batch, sequence, dim=12)
		'time_steps': combined_tt, # (batch, sequence)
		'mask': combined_mask, # (batch, sequence, dim=12)
		'labels': combined_labels # (batch, sequence, dim=11)
	}
...
if __name__ == '__main__':
	...
	dataset = PersonActivity(DATA_DIR / 'activity', download=True)
